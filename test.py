...
def handle_message(event):
    ...
    if channel.name == config['command_channel']:
        ...
        if result.message == '好':
            ...
            for member in data['group']['members']:
                feed = drinkbot.Feed(source="#slack", message=member)
                result = bot.hey(feed)
                if not result:
                    continue
                print '=== debug ==='
                ...
                print '=== debug end ==='
                slack.api_call('im.open', user=member)
                slack.rtm_send_message(result.to, result.message)
